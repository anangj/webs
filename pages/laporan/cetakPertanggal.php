...
// Pemasukan User
try{
$query2 = $conn->query("SELECT  SUM(jumlah) AS pemasukan FROM keuangan WHERE tanggal BETWEEN '$dari' AND '$sampai' AND status = 'Pemasukan' AND kodeUser = '$kode' "); 
...
// Pengeluaran User
$query3 = $conn->query("SELECT  SUM(jumlah) AS pengeluaran FROM keuangan WHERE tanggal BETWEEN '$dari' AND '$sampai' AND status = 'Pengeluaran' AND kodeUser = '$kode' ");
...
$query4 = $conn->query("SELECT ROUND ( SUM(IF(status = 'Pemasukan', jumlah, 0))-(SUM(IF( status = 'Pengeluaran', jumlah, 0))) ) AS subtotal FROM keuangan WHERE tanggal BETWEEN '$dari' AND '$sampai' AND kodeUser = '$kode' ");
...
$mpdf->SetTitle('Laporan Keuangan');
    ...
    <body>
        <p class="header">
            <span class="judul">~ RT Pintar ~</span><br /> 
            <span class="judul2">Laporan Keuangan</span>
        </p>
        <p class="body">
            &nbsp;&nbsp;&nbsp; Laporan Keuangan Anda, PerPeriode <b>Tanggal</b> Dalam menggunakan  aplikasi ini ,Detail Nama Pengguna Dari Laporan Yang Akan Di Cetak : 
            <table>
                <tr>
                    <td>Nama</td>
                    <td>:</td>
        ...
        <p class="body">
            &nbsp;&nbsp;&nbsp; Laporan Keuangan Dari Tanggal <b>'.$dari2.'</b> Sampai <b>'.$sampai2.'</b> :
            <table>
                <tr>
