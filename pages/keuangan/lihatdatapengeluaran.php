<?php 
    $query = $conn->query("SELECT * FROM keuangan WHERE status = 'Pengeluaran' AND kodeUser = '$userLogin' ORDER BY tanggal DESC LIMIT 10");
?>

<h1>Lihat Data Pengeluaran</h1>
<hr>
...
<div class="row">
    <div class="col-sm-6 mb-1">
        ...
        <form action="" method="post">
            <div class="input-group">
                <input type="date" id="datepicker" name="kunci" value="" class="form-control">
    ...
    <div class="col-sm-6 text-right mb-3">
        <a href="?page=keuangan&action=pengeluaran" class="btn btn-success"><i class="fa fa-sync-alt"></i>  Refresh</a>
        <a href="?page=keuangan" class="btn btn-success"><i class="fa fa-eye"></i>  Pemasukan</a>
...
<table class="table table-striped table-bordered table-sm">
    ...
    <?php 
             if(isset($_POST['cari'])){
                ...
                if($keyword == ""){
                    $query = $conn->query("SELECT * FROM keuangan WHERE status = 'Pengeluaran' AND kodeUser = '$userLogin' ORDER BY tanggal DESC");
                }else {
                    $query = $conn->query("SELECT * FROM keuangan WHERE status = 'Pengeluaran' AND tanggal LIKE '%$keyword%' ORDER BY tanggal DESC");
                }
            }
    ...
    <tfoot>
        <?php 
            ...
            if(isset($_POST['cari'])){
                ...
                if($keyword == ""){
                    ...
                    $result = $conn->query("SELECT  SUM(jumlah) AS pengeluaran FROM keuangan WHERE status = 'Pengeluaran' AND tanggal = '$keyword' AND kodeUser = '$userLogin'");  
                }
            }                 
        ...
        <tr>
            ...
            <?php
                $sql = $conn->query("SELECT * FROM keuangan WHERE status = 'Pengeluaran' AND kodeUser = '$userLogin' ");
                $count = $sql->rowCount();
            ...
            ?>
                Jumlah Pencatatan Data Pengeluaran:  <b><?= $count ?></b> Data yang di tampilkan, 10 data pencatatan terbaru.
            <?php
                }
            ?>
            </td>
